  ...
  return {
    ...
    createModal: function () {
      if (!this.modal) {
        ...
        element.find('.modal-title').text(this._('Lütfen İşlemi Onaylayın'));
        var content = this.options.content ||
                      this.options.i18n.content || /* Backwards-compatibility */
                      this._('Bu işlemi gerçekleştirmek istediğinize emin misiniz?');
        element.find('.modal-body').text(content);
        element.find('.btn-primary').text(this._('Confirm'));
        element.find('.btn-cancel').text(this._('İptal'));
